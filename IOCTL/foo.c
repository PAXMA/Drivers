...
#include "foo.h" //для работы с ioctl
...
static long device_ioctl(struct file *fl, unsigned int cmd, unsigned long arg){
	int i;
	char *temp;
	switch (cmd){
    	case IOCTL_CLEAR:
			for(i=0; i<BUF_LEN; i++)
				msg[i]=0;
			printk(KERN_INFO"IOCTL_CLEAR\nmsg: %s\n", msg);
			break;

		case IOCTL_CHANGE_BUF_LEN:
			temp = (char *) arg;
			sprintf(msg, "I dont know how to change size of an array to %s", temp);
			break;
	}
	return 0;
}
